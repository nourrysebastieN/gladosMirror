...
instance (Stream s, Hashable (Token s), Hashable (Chain s)) => Alternative (ParsecT s o m) where
    empty = ParsecT $ \s@(State _ _ off _) _ e _ ->
        e (Msg.error' (Single off) mempty Data.HashSet.empty) s
    (<|>) = plus
...
instance (Stream s, Monad m) => MonadFail (ParsecT s o m) where
    fail msg = ParsecT $ \s@(State _ _ off _) _ _ a ->
        a (Msg.error' (Single off) (Msg.Message msg) Data.HashSet.empty) s
