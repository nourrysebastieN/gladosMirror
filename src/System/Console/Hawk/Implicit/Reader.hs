...
reader x | A.isList x && not (A.isString x) = do
    ...
    return r{readerRead = \o s ->
        fmap (`A.cons` o) $ readerRead_ r s, readerFixup = A.reverse}
reader x | isAlgType x, [ctor] <- ctors x, [child] <- children x = do
    ...
    return r{
            readerFixup = \x -> up x $ readerFixup r $ down x
            ,readerRead = \x ->
                either Left (Right . up x) . readerRead r (down x)
            }
reader x = reader_ x

reader_ :: Any -> Maybe Reader
reader_ x | A.isString x =
    Just $ Reader "ITEM" False 1 id $ const $ Right . Any
reader_ x | typeName x == "Bool" =
    Just $ Reader "BOOL" True 1 id $ const $ \s ->
    maybe (Left $ "Could not read as boolean, " ++ show s)
        (Right . Any) $ parseBool s
...
reader_ x | res:_ <- catMaybes
    ...
    where
        ...
        f hlp t | typeOf (Any t) /= ty = Nothing
                | otherwise =
                    Just $ Reader hlp False 1 id $ const $ \s ->
                        case reads s of
                            [(x,"")] -> Right $ Any $ x `asTypeOf` t
                            _ ->
                                Left $ "Could not read as type "
                                ++ show (typeOf $ Any t) ++ ", " ++ show s
...
reader_ x | isAlgType x && length xs > 1 && all ((==) 0 . arity . snd) xs =
    Just $ Reader (map toUpper $ typeShell x) (typeName x == "Bool") 1 id
    $ const $ rd . map toLower
    where
        xs = [(map toLower c, compose0 x c) | c <- ctors x]

        rd s | null ys =
                Left $ "Could not read "
                ++ show s ++ ", expected one of: " ++ unwords (map fst xs)
             | Just (_,x) <- find ((==) s . fst) ys = Right x
             | length ys > 1 =
                Left $ "Ambiguous read for "
                ++ show s ++ ", could be any of: " ++ unwords (map fst ys)
...
reader_ x | isAlgType x, [c] <- ctors x, x <- compose0 x c = do
    ...
    return $ Reader (uncommas $ map readerHelp rs)
        (map readerBool rs == [True]) n id $ const $ \s ->
            let ss = commas s
            in if n == 1
                then fmap (recompose x . return)
                $ readerRead_ (head $ filter ((==) 1 . readerParts) rs) s
                else when' (length ss /= n)
                    (Left "Incorrect number of commas for fields")
                    *> (fmap (recompose x)
                    $ sequenceEither
                    $ zipWith readerRead_ rs
                    $ map uncommas $ takes (map readerParts rs) ss)

reader_ _ = Nothing

when' :: Applicative f => Bool -> f a -> f a
when' p s = if p then s else pure undefined
